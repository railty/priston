$sql = <<EOF
ALTER PROCEDURE [dbo].[Refresh_POS]
...
Declare @Message nvarchar(32);
	...
	Begin
		...
		If @Product_ID Is Null Begin
			...
			INSERT INTO Storage (Product_ID, InspectedDate, Store_ID, Units, StoragedDate, Active, LocationOnly)
			Select Dlt_Product.Prod_Num, GETDATE(), @Store_ID, 0, DATEADD(Day, -30, GETDATE()), 1, 0 From Dlt_Product Left Join Products On Dlt_Product.Prod_Num = Products.Prod_Num Collate Chinese_Taiwan_Stroke_CI_AS
			...
			-- Temporrary remark this one 11/30/2012 - enabled Dec 22, 2012 to send printing from POS to LableP
			...
			Delete from Pris.Dbo.OnSaleProduct;
			Insert Into Pris.Dbo.OnSaleProduct ([StartDate],[EndDate],[ProdNum],[TargetType],[TargetID],[MemberOnly],[D0],[D1],[D2],[D3],[D4],[D5],[D6])
			SELECT [StartDate],[EndDate],[ProdNum],[TargetType],[TargetID],[MemberOnly],[D0],[D1],[D2],[D3],[D4],[D5],[D6] FROM PM.MBPOSDB.dbo.OnSaleProduct;

			Delete from Pris.Dbo.OnSaleSchedule;
			Insert Into Pris.Dbo.OnSaleSchedule ([StartDate],[EndDate],[TargetType],[TargetID],[GroupDiscPct],[MemberOnly],[D0],[D1],[D2],[D3],[D4],[D5],[D6],[ModTimeStamp])
			SELECT [StartDate],[EndDate],[TargetType],[TargetID],[GroupDiscPct],[MemberOnly],[D0],[D1],[D2],[D3],[D4],[D5],[D6],[ModTimeStamp] FROM PM.MBPOSDB.dbo.OnSaleSchedule;
		End
		Else
		Begin
			...
			Begin
				...
				INSERT INTO Storage (Product_ID, InspectedDate, Store_ID, Units, StoragedDate, Active, LocationOnly)
				Select @Product_ID, GETDATE(), Store_ID, 0, DATEADD(day, -30, GETDATE()), 1, 0 From Stores Where Active = 'Y';
			End
			Else
			Begin
	...
	Begin
		...
		Select @Store_ID = Store_ID From Stores Where Active = 'Y';
		INSERT INTO Storage (Product_ID, InspectedDate, Store_ID, Units, StoragedDate, Active, LocationOnly)
		Select Prod_Num, GETDATE(), @Store_ID, 0, DATEADD(Day, -30, GETDATE()), 1, 0 From Storage Right Join Products On Storage.Product_ID = Products.Prod_Num 
		...
		DELETE Invoice_Item FROM Invoice_Item II INNER JOIN Products P On II.Prod_Num = P.Prod_Num
		Where Department Like '%meat%' Or Department Like 'Veg%' Or Department Like 'Fish%' Or Department Like 'Fru%';
		
		Execute Calculate_Sales;
		Delete from Pris.Dbo.Invoice_Item;
		...
		--this will create a 1 to 1 supplier <-> product table, whith the LAST supplier
		--Drop Table t_Temp2;
		--Select Product_ID, POs.PO_ID, POs.Supplier_ID Into t_Temp2 From POs 
		--Join PO_Details On POs.PO_ID = PO_Details.PO_ID;

		--Drop Table Supplier_Products;
		--Select A.Product_ID, A.Supplier_ID Into Supplier_Products From t_Temp2 A
		--Join (Select Product_ID, MAX(PO_ID) As PO_ID From t_Temp2 Group by Product_ID) B
		--On A.Product_ID = B.Product_ID And A.PO_ID = B.PO_ID;
		
		--this will create a 1 to many supplier <-> product table, whith the multiple supplier
		Drop Table Supplier_Products;
		Select Distinct Product_ID, Supplier_ID Into Supplier_Products From POs 
		Join PO_Details On POs.PO_ID = PO_Details.PO_ID
		Where PriceReceived Is NOT Null And PriceReceived > 0;
		
	End
	Print 'Finished Successfully!'
END
EOF
class AlterProcedureRefreshPos < ActiveRecord::Migration
  def up
	execute $sql
  end
